...
static void _clean_dir(const char *dir) {
	...
	while ((de = readdir(d)) != NULL) {
...
#if defined(_WIN32) && !defined(__MINGW32__) && !defined(__MINGW64__)
		{
			...
			if (!GetFileAttributesEx(de->d_name, GetFileExInfoStandard,
									 &data)) {
				continue;
			}
			if (data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
		...
		if (tmpdir_root == NULL) {
			...
			if (tmpdir_root == NULL) {
				...
				if (!gdTestIsDir(tmpdir_root)) {
					printf("tmpdir failed to be used or initialized (%s).",
						   tmpdir_root);
					exit(2);
				}
			}
...
char *gdTestTempFile(const char *template) {
...
#if defined(_WIN32) && !defined(__MINGW32__) && !defined(__MINGW64__)
	{
		...
		if (template == NULL) {
			error = GetTempFileName(tempdir, "gdtest", 0, ret);
			if (error = 0) {
		...
		for (x = 0; x < gdImageSX(buf_a); x++) {
			...
			if (c1 != c2) {
				...
				if (buf_diff)
					gdImageSetPixel(
						buf_diff, x, y,
						gdTrueColorAlpha(diff_r, diff_g, diff_b, diff_a));
			} else {
				if (buf_diff)
...
unsigned int gdMaxPixelDiff(gdImagePtr a, gdImagePtr b) {
	...
	for (x = 0; x < a->sx; x++) {
		for (y = 0; y < a->sy; y++) {
			...
			diff = max(diff,
					   abs(gdTrueColorGetAlpha(c1) - gdTrueColorGetAlpha(c2)));
			diff =
				max(diff, abs(gdTrueColorGetRed(c1) - gdTrueColorGetRed(c2)));
			diff = max(diff,
					   abs(gdTrueColorGetGreen(c1) - gdTrueColorGetGreen(c2)));
			diff =
				max(diff, abs(gdTrueColorGetBlue(c1) - gdTrueColorGetBlue(c2)));
		} /* for */
	} /* for */
...
int gdTestImageCompareToImage(const char *file, unsigned int line,
	...
	if (result.pixels_changed > 0) {
		...
		_gdTestErrorMsg(file, line,
						"Total pixels changed: %d with a maximum channel "
						"difference of %d.\n",
...
int gdTestImagePerceptualCompareToFile(const char *file, unsigned int line,
	...
	if (result.pixels_changed > max_pixels_changed) {
		...
		_gdTestErrorMsg(file, line,
						"Perceptual diff changed %u pixels; allowed %u with "
						"threshold %.4f.\n",
						result.pixels_changed, max_pixels_changed, threshold);
...
int gdNumFailures() { return failureCount; }
