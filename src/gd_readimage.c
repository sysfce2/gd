...
#define ENTRY(n, off, reader_fn, is_enabled, ...)                                                  \
    {                                                                                              \
        (n), (off), (const unsigned char[]){__VA_ARGS__},                                          \
            sizeof((const unsigned char[]){__VA_ARGS__}) / sizeof(unsigned char), (reader_fn),     \
            (is_enabled)                                                                           \
    }

static const struct FormatEntry format_table[] = {
    ENTRY("PNG", 0, GD_READIMAGE_PNG_READER, GD_READIMAGE_PNG_ENABLED, 0x89, 0x50, 0x4E, 0x47, 0x0D,
          0x0A, 0x1A, 0x0A),
    ENTRY("JXL", 0, GD_READIMAGE_JXL_READER, GD_READIMAGE_JXL_ENABLED, 0x00, 0x00, 0x00, 0x0C, 0x4A,
          0x58, 0x4C, 0x20, 0x0D, 0x0A, 0x87, 0x0A),
    ENTRY("JXL", 0, GD_READIMAGE_JXL_READER, GD_READIMAGE_JXL_ENABLED, 0xFF, 0x0A),
    ENTRY("JPEG", 0, GD_READIMAGE_JPEG_READER, GD_READIMAGE_JPEG_ENABLED, 0xFF, 0xD8, 0xFF),
    ENTRY("GIF", 0, gdImageCreateFromGifCtx, 1, 0x47, 0x49, 0x46, 0x38, 0x39, 0x61),
    ENTRY("GIF", 0, gdImageCreateFromGifCtx, 1, 0x47, 0x49, 0x46, 0x38, 0x37, 0x61),
    ENTRY("BMP", 0, gdImageCreateFromBmpCtx, 1, 0x42, 0x4D),
    ENTRY("TIFF", 0, GD_READIMAGE_TIFF_READER, GD_READIMAGE_TIFF_ENABLED, 0x4D, 0x4D, 0x00, 0x2A),
    ENTRY("TIFF", 0, GD_READIMAGE_TIFF_READER, GD_READIMAGE_TIFF_ENABLED, 0x49, 0x49, 0x2A, 0x00),
    ENTRY("WEBP", 8, GD_READIMAGE_WEBP_READER, GD_READIMAGE_WEBP_ENABLED, 0x57, 0x45, 0x42, 0x50),
    ENTRY("AVIF", 4, GD_READIMAGE_AVIF_READER, GD_READIMAGE_AVIF_ENABLED, 0x66, 0x74, 0x79, 0x70,
          0x61, 0x76, 0x69, 0x66),
    ENTRY("HEIC", 4, GD_READIMAGE_HEIF_READER, GD_READIMAGE_HEIF_ENABLED, 0x66, 0x74, 0x79, 0x70,
          0x68, 0x65, 0x69, 0x63),
    ENTRY("GD2", 0, GD_READIMAGE_GD2_READER, GD_READIMAGE_GD2_ENABLED, 0x67, 0x64, 0x32, 0x00),
    ENTRY("GD", 0, GD_READIMAGE_GD_READER, GD_READIMAGE_GD_ENABLED, 0xFF, 0xFE),
    ENTRY("GD", 0, GD_READIMAGE_GD_READER, GD_READIMAGE_GD_ENABLED, 0xFF, 0xFF),
    ENTRY("QOI", 0, gdImageCreateFromQoiCtx, 1, 0x71, 0x6F, 0x69, 0x66),
    ENTRY("XPM", 0, NULL, GD_READIMAGE_XPM_ENABLED, 0x2F, 0x2A, 0x20, 0x58, 0x50, 0x4D, 0x20, 0x2A),
    ENTRY("XBM", 0, NULL, 1, 0x23, 0x64, 0x65, 0x66, 0x69, 0x6E, 0x65, 0x20),
    {NULL, 0, NULL, 0, NULL, 0}};

static int sig_match(const unsigned char *probe, size_t probe_len, const struct FormatEntry *e)
{
...
static const struct FormatEntry *find_format(const unsigned char *probe, size_t probe_len)
{
    size_t i;
...
static void report_unreadable_format(const struct FormatEntry *e, const char *source)
{
    if (e == NULL) {
    ...
    if (!e->enabled) {
        gd_error("gd-readimage: %s image support has been disabled", e->name);
        return;
    }

    gd_error("gd-readimage: %s image does not support reading from %s", e->name, source);
}
